...
import {
  partition, isEnabled, projectDir, listProjects, walk,
} from '../lib.mjs';
...
export default {
  ...
  run(ctx) {
    ...
    const lines = [
      ...
      `enabled    : ${enabled ? 'yes (hooks enforce)' : 'no (hooks fail-open — run: enable)'}`,
    ...
    return {
      ok: true,
      lines,
      data: { vaultRoot: ROOT, project: PROJECT, projectDir: projectDir(), enabled, notes, projects },
    };
  },
};
